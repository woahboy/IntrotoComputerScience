        ...
        for page in graph:
            ...
            for node in graph:
                if page in graph[node]:
                    # if max(path_length(page, node, graph), path_length(node, page, graph)) > k:
...
# def path_length(node, page, graph):
#     length = 1
#     toSearch = []
#     searched = []
#     [toSearch.append(x) for x in graph[node]]
#     if page == node and page in graph[node]:
#         return 0
#     while page not in graph[node]:
#         length += 1
#         searched.append(node)
#         for elem in toSearch:
#             tempList = []
#             if elem not in searched:
#                 if page in graph[elem]:
#                     return length
#                 searched.append(elem)
#             for item in graph[elem]:
#                 if item not in toSearch:
